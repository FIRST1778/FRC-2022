...
/**
 * Faster line movement of length in meters at angle direction in radians
 *
 * @property length
 * @property angle
 * @constructor Create a line of length meters at angle direction in radians
 */
class FastLine(private var length: Double, private var angle: SIUnit<Radian>) : PathSegment(){
    ...
    override fun execute(timer: Timer): Boolean {
        return if(!turnComplete) {
            ...
            if(length > 0) {
                Drive.Autonomous.driveForwardsFast()
            } else if(length < 0) {
                Drive.Autonomous.driveBackwardsFast()
            }
            false
        } else {
            Drive.stop()
